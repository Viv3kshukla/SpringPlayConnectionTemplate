...
// @LINE:10
private[this] lazy val controllers_FirstController_sayHello3_route = Route("GET", PathPattern(List(StaticPart(Routes.prefix),StaticPart(Routes.defaultPrefix),StaticPart("helloUser"))))
private[this] lazy val controllers_FirstController_sayHello3_invoker = createInvoker(
play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.FirstController]).sayHello,
HandlerDef(this.getClass.getClassLoader, "", "controllers.FirstController", "sayHello", Nil,"GET", """""", Routes.prefix + """helloUser"""))
        
def documentation = List(("""GET""", prefix,"""controllers.Application.index"""),("""GET""", prefix + (if(prefix.endsWith("/")) "" else "/") + """sum/$password<[^/]+>""","""controllers.SumController.socket(password:String)"""),("""GET""", prefix + (if(prefix.endsWith("/")) "" else "/") + """assets/$file<.+>""","""controllers.Assets.versioned(path:String = "/public", file:Asset)"""),("""GET""", prefix + (if(prefix.endsWith("/")) "" else "/") + """helloUser""","""@controllers.FirstController@.sayHello""")).foldLeft(List.empty[(String,String,String)]) { (s,e) => e.asInstanceOf[Any] match {
...
// @LINE:10
case controllers_FirstController_sayHello3_route(params) => {
   call { 
        controllers_FirstController_sayHello3_invoker.call(play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.FirstController]).sayHello)
   }
}
